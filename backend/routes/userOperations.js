...
userroute.post("/filterusersearch", async (req, res, next) => {
  ...
  //console.log("UserID: ", user_id)
  await User.find(
    { $or: [{ fname: req.body.name }, { city: req.body.name }] },
    (error, result) => {
      if (error) {
        res.writeHead(500, {
          "Content-Type": "text/plain",
...
userroute.post("/uviewcart", (req, res) => {
  ...
  Order.find(
    { user_id: req.body.user_id, orderstatus: " " },
    {},
    (error, result) => {
      if (error) {
        console.log(error);
...
userroute.post("/createorder", async (req, res) => {
  ...
  try {
    ...
    await Order.updateOne(
      { _id: req.body.order_id, "cart.user_id": req.body.user_id },
      {
        $set: { "cart.$.cartstatus": "done" },
